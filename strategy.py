...
class Momentum(bt.Indicator):
    lines = ('trend',)
    params = (('wMean', 30),('wStock', 5))
    indx = ('indx',)
    # params = (('wStock', 5),)

    def __init__(self, indx):
        self.addminperiod(self.params.wMean + self.p.wStock)
        self.indx = indx

    def next(self):
        data = self.data.get(size= self.p.wMean + self.p.wStock)
        momentum = np.median([getMomentum(data[i:i+5]) for i in range(len(data)-5)])
        # momentum = data.rolling(self.p.wStock).apply(getMomentum, raw=False)[self.p.wStock:]
        try:
            self.lines.trend[0] = momentum
        except:
...
class TestStrategy(bt.Strategy):
    def __init__(self):
        ...
        for i,d in enumerate(self.stocks):
            ...
            # if i == 164:
            #     import pdb;pdb.set_trace()
            self.inds[d]["momentum"] = Momentum(d.close, indx = i)
    ...
    def rebalance_portfolio(self):
        ...
        self.rankings = self.stocks#list(filter(lambda d: len(d.open) > 100, self.stocks))
